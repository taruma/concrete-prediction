...
Source code dokumen ini bisa diakses di GitHub [taruma/concrete-prediction](https://github.com/taruma/concrete-prediction). 
...
Beberapa bagian yang telah dihapus akan dilakukan dibagian @sec-revision. Dan untuk penambahan seperti variasi model lainnya maupun diskusi terkait akan dilakukan di bab ini atau setelahnya.

## Evaluasi Model {#sec-rev-evaluate}

Dari @tbl-model-evaluation-compare, diketahui hanya `model_rf3` saja yang memiliki nilai `MAE <= 4` dengan `R2 >= 90%`. Sedangkan untuk `model_lm0` dan `model_nn4` nilai `R2` tidak mencapai sampai `90%`. Pada bagian revisi ini, akan dilakukan prediksi dengan dataset testing yang pada bagian sebelumnya telah di hilangkan. 

### Prediksi dataset _testing_ {#sec-rev-predict-test}

Prediksi menggunakan data dari `test0` dengan nilai aktualnya diperoleh dari kolom `strength` atau `test0$strength`. Hasil disajikan serupa menggunakan grafik R2 untuk setiap modelnya yang dapat dilihat di @fig-rev-testing-compare. 

```{r}
#| echo: false
#| output: false

pred_test_lm0 <- predict(model_lm0, test0)
pred_test_rf3 <- predict(model_rf3, test0)
pred_test_nn4 <- predict(model_nn4, as.matrix(test0 |> select(-id, -strength)))
```
...
#| column: screen-inset
#| message: false
#| label: fig-rev-testing-compare
#| layout-ncol: 3
#| fig-cap: Grafik R2 setiap model untuk data _test_
#| fig-subcap: 
#|   - "`model_lm0`"
#|   - "`model_rf3`"
#|   - "`model_nn4`"

myplot_scatter_pred(pred_test_lm0, test0$strength, test0$age |> exp() |> factor(levels = levels_age))
myplot_scatter_pred(pred_test_rf3, test0$strength, test0$age |> exp() |> factor(levels = levels_age))
myplot_scatter_pred(pred_test_nn4, test0$strength, test0$age |> exp() |> factor(levels = levels_age))
```

Dari @fig-rev-testing-compare, model terbaik diperoleh oleh model `model_rf3` dan `model_nn4` yang diketahui nilai $R$ sebesar $0.94$ (Untuk memperoleh nilai $R^2$, tinggal dikuadratkan saja). Berikut hasilnya dalam bentuk tabel @tbl-rev-testing-compare. 

```{r}
#| echo: false
#| column: body
#| label: tbl-rev-testing-compare
#| tbl-cap: Metrik model untuk prediksi _validation_

mmae_test_lm0 <- MAE(pred_test_lm0, test0$strength)
mmae_test_rf3 <- MAE(pred_test_rf3, test0$strength)
mmae_test_nn4 <- MAE(pred_test_nn4, test0$strength)

mr2_test_lm0 <- R2(pred_test_lm0, test0$strength)
mr2_test_rf3 <- R2(pred_test_rf3, test0$strength)
mr2_test_nn4 <- R2(pred_test_nn4, test0$strength)

data.frame(
  model = c("model_lm0", "model_rf3", "model_nn4"),
  MAE = c(mmae_test_lm0, mmae_test_rf3, mmae_test_nn4),
  R2 = c(mr2_test_lm0, mr2_test_rf3, mr2_test_nn4)
) |> knitr::kable()

```

Dari @tbl-rev-testing-compare diperoleh bahwa `model_nn4` lah yang memperoleh nilai terbaik dengan nilai $R^2 = `r mr2_test_nn4`$ dan $\text{MAE} = `r mmae_test_nn4`$. Target awal dari capstone tidak tercapai dari semua model yaitu `MAE <= 4` dan `R2 >= 90%`. Sehingga masih dapat dilakukan peningkatan model untuk memperoleh hasil yang ingin dicapai. Saran yang disampaikan di @sec-closing akan diterapkan nantinya.
...
- 2022-12-31
  - Versi `open-source` dokumen ini dirilis di GitHub [taruma/concrete-prediction](https://github.com/taruma/concrete-prediction). Modelnya juga telah disertakan.
  - Ditambah bagian @sec-revision untuk melanjutkan lembar kerja ini. 
- 2022-12-22
